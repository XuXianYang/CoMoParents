...
-(void)setUpLoginView
{
    ...
    [_remberNameAndPasswordBtn setImage:[UIImage imageNamed:@"noremberName_p"] forState:UIControlStateNormal];
    [_remberNameAndPasswordBtn setImage:[UIImage imageNamed:@"remberName_p"] forState:UIControlStateSelected];
    ...
    [_loginBtn setBackgroundColor:XXYMainColor];
...
-(void)loginBtnClicked:(UIButton*)btn
{
    ...
    [BSHttpRequest POST:requestUrl parameters:@{@"username":_nameTextField.text,@"password":_passwordTextField.text} success:^(id responseObject){
        ...
        {
            if([objString[@"data"][@"user"][@"type"] isEqualToNumber:@3])
            {
                ...
                if(objString[@"data"][@"user"][@"parentInfo"][@"hasChild"]&&objString[@"data"][@"user"][@"parentInfo"][@"defaultChildUserId"])
                {
                   [self getUserToken];
            ...
            {
                [self setUpAlertController:@"该客户端只支持家长登录!"];
            }
        }
        else
        {
