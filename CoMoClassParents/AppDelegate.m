            ...
            else
            {
                ...
                [BSHttpRequest GET:requestUrl parameters:@{@"sid":userSid} success:^(id responseObject){
                   ...
                   // NSLog(@"145-Success: %@",[[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding]);
                    ...
                    if(objString[@"data"][@"parentInfo"][@"hasChild"]&&objString[@"data"][@"parentInfo"][@"defaultChildUserId"])
                    {
                        [self getUserToken];
                        self.window.rootViewController=tabbarCon;
                        [self.window makeKeyAndVisible];
...
//    NSString *subtitle = content.subtitle;  // 推送消息的副标题
    ...
    if([response.notification.request.trigger isKindOfClass:[UNPushNotificationTrigger class]]) {
        ...
        {
            XXYSafetyToolController *chat = [[XXYSafetyToolController alloc]init];
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"CoMoClassParents"];
